...
static void SendTxData(void)
{
  ...
  if (LmHandlerIsBusy() == false)
  {
    ...
    if(0 != LSM6DSO_USER_Acc_GetAxes(&acc_x, &acc_y, &acc_z)){
    	printf("Acc not working\n");
    }
    if(0 != LSM6DSO_USER_Gyro_GetAxes(&gyr_x, &gyr_y, &gyr_z)){
        printf("Gyro not working\n");
    }
    */

    //temperature = (int16_t)(sensor_data.temperature);
    //pressure = (uint16_t)(sensor_data.temperature * 100 / 10); /* in hPa / 10 */
